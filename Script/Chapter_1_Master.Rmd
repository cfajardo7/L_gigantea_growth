...
#I keepgetting this error message when I attempt to add a column with the function of the species to the data frame
#Error in `mutate()`:
#ℹ In argument: `functionality = case_when(...)`.
#Caused by error in `case_when()`:
#! `..1 (left)` must be a logical vector, not the string "M_lugubris".
#Backtrace:
#1. ... %>% ...
#9. dplyr::case_when(...)
```
