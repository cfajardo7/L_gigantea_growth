...
by_sex_fall_win_data <- master_sl %>% #calling master shell length data
  ...
  #reframe(limpet_growth = (SL_mm[Season=="Winter"]-SL_mm[Season=="Fall"])/(Date[Season == "Winter"]-Date[Season=="Fall"])*30) %>%  #this is to get true growth rate using dates calculated from lubridate then mulitplied by 30 to get growth per month
  reframe(limpet_growth = (SL_mm[Season=="Winter"]-SL_mm[Season=="Fall"])/4.75) %>% 
  left_join(master_sl %>% #this is to keep original lenght for fall
              filter(Tag_ID!="0",
                     Season=="Fall") %>% 
...
complete_growth <- bind_rows(by_sex_fall_win_data, Spring_by_sex_data)
...
comp_growth_scatter <- complete_growth %>% 
  ...
  geom_smooth(method = "lm",
              formula = "y~poly(x,2)") #this adds a quadratic function
...
I'm not sure what to do with this chunk of code, I copied it from 2023-2024 Growth visualization markdown. Will tackle later.
```{r, Look at later}
#Density?
...
#Analyis of Population Distribution Data of All Seasons
