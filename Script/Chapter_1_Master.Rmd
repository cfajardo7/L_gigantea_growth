...
I'm leaving this chunk in for reference to code with geom_smooth
...
#this a histogram with all of the shell lengths recorded for all three seasons
all_seas_hist <- size_dist_all_seasons %>% #calling df with shell length data
  ggplot( aes(x=SL_mm)) + #Sl on x, I am not selecting y so R can use limpet count for density plot
  geom_histogram(fill="orange",#color of bars
           colour="black",#color of bar outline
           alpha= 1,#transparency of bar
           binwidth = 1) + #size of bars
  labs(title = "Shell Length Distribution September 23-June 24",
       x = "Shell Length (mm)",
       y = "Owl Limpet Count")+
  theme(axis.title.y = element_blank(),
        axis.title.x = element_text(size = 10),
...
sl_med_SE <- ggplot(shell_lenght_stats_per_seas,
  ...
  geom_point(show.legend = FALSE,
             size = 5)+ #this adds the point where the median is 
  geom_errorbar(aes(ymin = mean_sl - sd_sl, ymax = mean_sl + sd_sl),
                   width = 0.2,
                show.legend = FALSE)+ #this add the error line which is the mean of the shell length +/- the standard deviation
  geom_jitter(data = size_dist_all_seasons,
              aes(x = Season,
                  y = SL_mm),
              alpha = 0.1,
              width = 0.15,
...
# Big or Small Limpets For All Seasons and Individual Seasons
```{r, Big or Small Limpets For All Seasons and Individual Seasons}
big_small_for_all_seasons <- master_sl %>%
  filter(Season!="Spring",
         Date!="2023_12_10") %>% 
  mutate(Big_small=ifelse(SL_mm>= 32, "big", "small"))

all_seas_big_small_density_plot <- ggplot(big_small_for_all_seasons,
                                   aes(x=Big_small,
                                       y=SL_mm,
                                       fill = Big_small))+
...
all_seas_big_small_density_plot

only_smalls <- big_small_for_all_seasons %>% 
  filter(Big_small!="big")

only_smalls_boxplot <- ggplot(only_smalls,
                              aes(x=Season,
                                  y = SL_mm,
                                  fill = Season))+
  geom_boxplot()
...
#i'm going to stick to the generalized mixed effects model because the data is not normally distributed and the data is continous.
#glmm
...
#When after mutating the sum the number of individuals, I get a new row called spec_sum which gives me the total number of individuals of all species into one column/transect. For instance, in plot name CNM_1_C_5, spec_sum adds all of the number across that row, so the sum is 2. If I pivot longer, species sum, which corresponds to one transect on either a yes or no plot, shows up often making it difficult to visualize. I might need to sperate the data by seasons to view the species sum by transect
...
long_mobiles <- mobiles_master %>% 
  ...
  species %in% "M_lugubris" ~ "carnivore",
...
# Summary of mobiles
```{r, Summary of mobiles}
summary_mobiles <- long_mobiles %>% 
  mutate(log_count = log(count+1)) %>% 
  group_by(functionality, Season, COL_pres) %>% 
  summarize(mean_count = mean(log_count, na.rm = TRUE),
          se_count = sd(log_count, na.rm = TRUE)/sqrt(n()))
summary_mobiles

summary_plot <- summary_mobiles %>% 
  ggplot(aes(x = COL_pres,
             y = mean_count,
             color = functionality))+
  geom_point(position = position_dodge(width = 0.3))+ #this code helps spread apart the error bars
  geom_errorbar(aes(ymin=mean_count-se_count, ymax=mean_count+se_count),
               width = 0.1,
               position = position_dodge(width = 0.3))+ #this code helps spread apart the error bars
  facet_wrap(~Season)
summary_plot
```

# linear models with random effect per function
```{r, linear models per funtion}
meso_lm <- lmer(log(count+1)~COL_pres+(1|Transect),
                data = long_mobiles %>% 
                  filter(functionality == "mesograzer", #change function
                         Season == "Spring")) #change season
check_model(meso_lm)
ggsave(here("Output", "meso_lm_Check_Model.png"))
anova(meso_lm)
#When Nyssa and I ran the mixed effects model with season as an interaction term, seasonality was so strong that if there was any effect by COL_pres it is not noticeable. So we took out season as an interaction, and filter out each season and look at the results. These are the results:
    #function mesograzer
        #Season: Fall
            #-residuals not normal, p= 0.001
        #Season: Winter
            #residuals not normal, p= 0.81
        #Season: Spring
            #residuals not normal, p= 0.25
            #Residuals are not normal. Will run glmer, family poisson, log link

macro_lm <- lmer(log(count+1)~COL_pres+(1|Transect),
                data = long_mobiles %>% 
                  filter(functionality == "macrograzer", #change function
                         Season == "Spring")) #change season
check_model(macro_lm)
ggsave(here("Output", "macro_lm_check_model.png"))
anova(macro_lm)
#I ran the mixed effects model with season as an interaction term, seasonality was so strong that if there was any effect by COL_pres it is not noticeable. Took out season as an interaction, and filter out each season and look at the results. These are the results:
    #function macrograzer
        #Season: Fall
            #-residuals not normal, p= 0.86
        #Season: Winter
            #residuals not normal, p= 0.017
        #Season: Spring
            #residuals not normal, p= 0.42
            #Residuals are not normal. Will run glmer, family poisson, log link

carn_lm <- lmer(log(count+1)~COL_pres+(1|Transect),
                data = long_mobiles %>% 
                  filter(functionality == "carnivore", #change function
                         Season == "Spring")) #change season
check_model(carn_lm)
ggsave(here("Output", "carn_lm_Check_Model.png"))
anova(carn_lm)
#I ran the mixed effects model with season as an interaction term, seasonality was so strong that if there was any effect by COL_pres it is not noticeable. Took out season as an interaction, and filter out each season and look at the results. These are the results:
    #function carnivore
        #Season: Fall
            #-residuals not normal, p= 0.12
        #Season: Winter
            #residuals not normal, p= 0.98
        #Season: Spring
            #residuals not normal, p= 0.67
            #Residuals are not normal. Will run glmer, family poisson, log link
```
# Generalized Linear Model Per Function, poisson log link
```{r, Generalized Linear Model Per Function, poisson}
meso_glmm <- glmer((count+1)~COL_pres*Season+(1|Transect),
                data = long_mobiles, 
                family = poisson(link = "log")) #%>% 
                  #filter(functionality == "mesograzer")) #change function
check_model(meso_glmm)
ggsave(here("Output", "glmm_func_cm.png"))
anova(meso_glmm)
summary(meso_glmm)
```
...
# Types of grazers
```{r, Types of grazers}
grazers_types <- long_mobiles %>% 
  filter(functionality == "macrograzer"|functionality == "mesograzer")%>% #using | helps filter out two types of variables in the same column
  mutate(grazer_type = case_when(
  species %in% "L_hartwegii" ~ "chiton",
  species %in%  "L_dentiens" ~ "chiton",
  species %in%  "L_plena" ~ "littorine",
  species %in%  "L_keenae" ~ "littorine",
  species %in%  "L_limatula" ~ "limpet",
  species %in%  "L_scabra" ~ "limpet",
  species %in%  "Nuttallina_sp" ~ "chiton",
  species %in%  "Mopalia_sp" ~ "chiton",
  species %in%  "T_eiseni" ~ "tegula_snail",
  species %in%  "T_funebralis" ~ "tegula_snail",
  species %in%  "T_gallina" ~ "tegula_snail",
  species %in%  "L_gigantea" ~ "limpet"))
```

# Summary and Visual of Grazer Type
```{r}
summary_grazers <- grazers_types %>% 
  mutate(log_count = log(count+1)) %>% 
  group_by(grazer_type, Season, COL_pres) %>% 
  summarize(mean_count = mean(log_count, na.rm = TRUE),
          se_count = sd(log_count, na.rm = TRUE)/sqrt(n()))

grazer_summary_plot <- summary_grazers %>% 
  ggplot(aes(x = COL_pres,
             y = mean_count,
             color = grazer_type))+
  geom_point(position = position_dodge(width = 0.3))+ #this code helps spread apart the error bars
...
# Linear Models for Grazer Type
```{r, Linear Models for Grazer Type}
limpet_lm <- lmer(log(count+1)~COL_pres*Season+(1|Transect),
                data = grazers_types %>% 
                  filter(grazer_type == "limpet")) #change type
                        
check_model(limpet_lm)
ggsave(here("Output", "lmer_type_cm.png"))#when I run the check function, I'm having a hard time seeing it
#Residuals are not normal, should I get rid of interaction? Or run glmer instead?
anova(limpet_lm)
```

# Trophic Type Graph
```{r, Trophic Type Graph}
trophic_type_mobiles <- long_mobiles %>% 
  ggplot(aes(x = COL_pres,
             y= count,
             color = functionality)) +
  geom_point()+
  theme()+
  facet_wrap(~Season)
trophic_type_mobiles
```


# By species graph
```{r, By species graph}
by_species_mobiles <- long_mobiles %>% 
  ggplot(aes(x = Season,
             y = count,
             fill = species)) +
  geom_bar(position="fill", stat="identity")+
  scale_fill_viridis_d(option = "H")+ #i like this graph but look for better fill colors
  theme()+
  facet_wrap(~COL_pres)
by_species_mobiles
...
# Small owl Limpets Present in Presence of Large Owl Limpet
```{r, Small owl Limpets Present in Presence of Large Owl Limpet}
smalls_with_big <- long_mobiles %>% 
  filter(species=="L_gigantea")

small_with_big_boxplot <- ggplot(smalls_with_big,
                                 aes(x=Season,
                                     y= count,
                                     fill = COL_pres))+
  geom_boxplot()
small_with_big_boxplot 
```


# Temperature Data
I need to work out this chunck
```{r, Temperature data}
...
#ggplot(temperature_by_season, aes(x =Season, y = ))+
 # geom_line()+
 # geom_point(aes(color = Season))
```
