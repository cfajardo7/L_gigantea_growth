...
### Size (`sl_mm`) by season for small individuals
...
p_small_size <- limpet_size %>%
  ...
  geom_jitter(width = 0.08, height = 0, alpha = 0.20, size = 0.8) +
  labs(x = NULL, y = "Shell length (mm)",
       title = "Owl limpet size distribution by season for individuals less than 30 mm"
       ) +
  scale_y_continuous(expand = expansion(mult = c(0.02, 0.08)))+
   scale_color_manual(values = season_colors)+
  scale_fill_manual(values = season_colors)+
...
#### Gamma GLMM with random effects for plot for small individuals

```{r}
small_sl <- limpet_size %>% 
  filter(size == "small")

m_small_size_gamma <- glmmTMB(
  sl_mm ~ season + (1 | plot_name),
  dispformula = ~ season,  # Model dispersion as a function of season because unequal variances
  family = Gamma(link = "log"), # better but still not the best, but I trust more this model
  data = small_sl
)

summary(m_small_size_gamma)
```

##### Diagnostics for small shell length

```{r}
small_sim_size <- DHARMa::simulateResiduals(m_small_size_gamma, n = 1000)
plot(small_sim_size)
DHARMa::testDispersion(small_sim_size)
```
##### Estimated marginal means for small individuals(back-transformed to mm)

```{r}
small_emm_size <- emmeans(m_small_size_gamma, ~ season, type = "response")
small_emm_size
...
p_small_size_emm <- ggplot(df_small_size_emm, aes(x = season, y = response)) +
  ...
  geom_point(size = 2) +
  geom_errorbar(data = df_small_size_emm, aes(ymin = asymp.LCL, ymax = asymp.UCL), width = 0.12) +
  scale_fill_manual(values = season_colors)+
  scale_color_manual(values =season_colors)+
  geom_signif(comparisons = list(c("Fall", "Spring")),
              map_signif_level = TRUE, # Automatically uses asterisks for p-values
   ...
   geom_signif(comparisons = list(c("Winter", "Spring")),
              ...
              annotations = "***")+
  labs(x = NULL, y = "Estimated mean shell length (mm)",
       title = "Seasonal effect on small owl limpet size (Gamma GLMM)"
       ) +
  scale_y_continuous(expand = expansion(mult = c(0.02, 0.08)))+
  theme(legend.position = "none")
ggsave(filename = "smallSL.png", width = 14, height = 6)

p_small_size_emm
```
### Size (`sl_mm`) by season for large individuals
...
p_large_size <- limpet_size %>%
  ...
  geom_jitter(width = 0.08, height = 0, alpha = 0.20, size = 0.8) +
  labs(x = NULL, y = "Shell length (mm)",
       title = "Owl limpet size distribution by season for individuals greater than 30 mm"
       ) +
  scale_y_continuous(expand = expansion(mult = c(0.02, 0.08)))+
   scale_color_manual(values = season_colors)+
  scale_fill_manual(values = season_colors)+
...
summary(m_large_size_gamma)
```

##### Diagnostics for large shell length

```{r}
large_sim_size <- DHARMa::simulateResiduals(m_large_size_gamma, n = 1000)
plot(large_sim_size)
DHARMa::testDispersion(large_sim_size)
```

##### Estimated marginal means for small individuals(back-transformed to mm)

```{r}
large_emm_size <- emmeans(m_large_size_gamma, ~ season, type = "response")
large_emm_size
...
p_large_size_emm <- ggplot(df_large_size_emm, aes(x = season, y = response)) +
  ...
  geom_point(size = 2) +
  geom_errorbar(data = df_large_size_emm, aes(ymin = asymp.LCL, ymax = asymp.UCL), width = 0.12) +
  scale_fill_manual(values = season_colors)+
  scale_color_manual(values =season_colors)+
  geom_signif(comparisons = list(c("Fall", "Spring")),
              map_signif_level = TRUE, # Automatically uses asterisks for p-values
   ...
   geom_signif(comparisons = list(c("Winter", "Spring")),
              ...
              annotations = "***")+
  labs(x = NULL, y = "Estimated mean shell length (mm)",
       title = "Seasonal effect on large owl limpet size (Gamma GLMM)"
       ) +
  scale_y_continuous(expand = expansion(mult = c(0.02, 0.08)))+
  theme(legend.position = "none")
ggsave(filename = "largeSL.png", width = 14, height = 6)
...
```
##### Physical Metrics
Because my work is heavily tied to physical changes that is correlated to seasonal change, I have pulled temperature and upwelling data from NOAA's ERDAP data. The upwelling data that I used is the CUTI(Coastal Upwelling Transport Index). Temperature data is sea surface temperature that is collected from buoys.
```{r}
# CUTI Data
# This pulls data
