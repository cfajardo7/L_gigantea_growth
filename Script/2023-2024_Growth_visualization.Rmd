...
```{r, Simple Statistics}
Feb_data <- filter(master_sl, Date=="2024_02")
Sep_data <- filter(master_sl, Date == "2023_09")
Dec_data <- filter(master_sl, Date == "2023_12")
...
```{r}
sep_SL_mean <- mean(Sep_data$SL_mm)
dec_SL_mean <- mean(Dec_data$SL_mm)
feb_SL_mean <- mean(Feb_data$SL_mm)
...
feb_cnm_pop_density <- Feb_data %>% #calling df with shell length data
  ...
  scale_y_continuous(limits = c(0,60))+ #lines I want to show up on y axis
  geom_vline(xintercept = mean(Feb_data$SL_mm),
...
feb_tagged_limpets <- Feb_data %>% #named new data frame using shell length data
...
This chunk gives population size distribution of tagged limpets in February
```{r, #Tagged Limpets Shell Length Distribution Graph}
feb_tagged_distribution <- Feb_data %>% #calling data frame with tagged limpets only
  select(Limpet_No, SL_mm) %>% #selecting limpet number to count and shell length
  ggplot( aes(x=SL_mm)) + #Sl on x, I am not selecting y so R can use limpet count for density plot
