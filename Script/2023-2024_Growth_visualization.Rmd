...
Winter_data <- filter(master_sl, Season == "Winter",
                      Date != "2023_12", 
                      Plot_Name != "CNM_7") #Normalized to 6 full transects
Fall_data <- master_sl %>% 
...
Spring_data <- filter(master_sl, Season == "Late_Spring",
                      Plot_Name != "CNM_7", #Normalized to 6 full transects
                      Plot_Name != "CNM_8") #Normalized to 6 full transects
...
```{r, Population Shell Length Distribution Graph for Winter}
...
```{r, Size Distribution of Tagged Limpets In Spring (June)}
...
```{r, Population Average Growth rate of Tagged Limpets from March to June}
